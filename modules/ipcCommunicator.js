...
ipc.on('backendAction_importWalletFile', (e, path, pw) => {
    ...
    const binPath = ClientBinaryManager.getClient('weth').binPath;
    const nodeProcess = spawn(binPath, ['wallet', 'import', path]);

    nodeProcess.once('error', () => {
        error = true;
        e.sender.send('uiAction_importedWalletFile', 'Couldn\'t start the "weth wallet import <file.json>" process.');
    });
