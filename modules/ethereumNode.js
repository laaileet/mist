...
class EthereumNode extends EventEmitter {
     ...
     * @param  {String} nodeType weth, eth, etc
    ...
    _start(nodeType, network, syncMode) {
        ...
        return this.stop()
            ...
            .catch((err) => {
                ...
                // if unable to start eth node then write weth to defaults
                if (nodeType === 'eth') {
                    Settings.saveUserData('node', 'weth');
                }
            ...
            logRotate(Settings.constructUserDataPath('node.log'), { count: 5 }, (err) => {
                    ...
                    if (STATES.STARTING === this.state) {
                        ...
                        if (nodeType === 'weth') {
                            if (dataStr.indexOf('fatal: error') >= 0) {
                                const error = new Error(`Weth error: ${dataStr}`);
                ...
                this.once('data', () => {
                    /*
                        ...
                        errors, etc (see weth port-binding error above)
                    */
                    setTimeout(() => {
                        if (STATES.STARTING === this.state) {
