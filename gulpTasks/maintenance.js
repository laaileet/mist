...
gulp.task('update-nodes', (cb) => {
    const clientBinariesWeth = clientBinaries.clients.Weth;
    const localWethVersion = clientBinariesWeth.version;
    const newJson = clientBinaries;
    const weth = newJson.clients.Weth;

    // Query latest weth version
    ...
    .then((tagName) => {
        const latestWethVersion = tagName.match(/\d+\.\d+\.\d+/)[0];

        // Compare to current weth version in clientBinaries.json
        if (cmp(latestWethVersion, localWethVersion)) {
            weth.version = latestWethVersion;
            ...
            .then((hash) => {
                ...
                got('https://wethstore.blob.core.windows.net/builds?restype=container&comp=list', { xml: true })
                .then((response) => {
                    ...
                    _.keys(weth.platforms).forEach((platform) => {
                        _.keys(weth.platforms[platform]).forEach((arch) => {
                            // Update URL
                            let url = weth.platforms[platform][arch].download.url;
                            url = url.replace(/\d+\.\d+\.\d+-[a-z0-9]{8}/, `${latestWethVersion}-${hash}`);
                            weth.platforms[platform][arch].download.url = url;

                            // Update bin name (path in archive)
                            let bin = weth.platforms[platform][arch].download.bin;
                            bin = bin.replace(/\d+\.\d+\.\d+-[a-z0-9]{8}/, `${latestWethVersion}-${hash}`);
                            weth.platforms[platform][arch].download.bin = bin;

                            // Update expected sanity-command version output
                            weth.platforms[platform][arch].commands.sanity.output[1] =
                            String(latestWethVersion);

                            // Update md5 checksum
                            blobs.forEach((blob) => {
                                if (String(blob.Name) === _.last(weth.platforms[platform][arch].download.url.split('/'))) {
                                    const sum = new Buffer(blob.Properties[0]['Content-MD5'][0], 'base64');

                                    weth.platforms[platform][arch].download.md5 = sum.toString('hex');
                                }
                            });
                        });
                    });
